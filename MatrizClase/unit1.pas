...
type
  ...
  // Nodo cabecera (filas y columnas)
  ...
  TForm1 = class(TForm)
    ...
    procedure GraficarButtonClick(Sender: TObject);
...
procedure TForm1.InicializarMatriz;
begin
  New(root);
  Root^.Fila := '';
  Root^.Columna := '';
...
procedure TForm1.GraficarButtonClick(Sender: TObject);
var
  Dot: Text;
  ...
  begin
    ...
    // Colocar el tamaño de filas y columnas al array
    ...
    // Realizar conexiones primero con root
    if ColCount > 0 then
    begin
    ...
    // Realizar conexiones con las demas columnas
    ...
    // Realizar conexiones con las demas filas
    ...
    begin
      ...
      begin
        Write(Dot, Format('  { rank=same; rowHeader_%s; f_%s_c_%s', [Filas[i]^.Fila, Filas[i]^.Fila, ActualNodo^.Columna]) );
        Writeln(Dot, ' }');
...
procedure TForm1.AgregarButtonClick(Sender: TObject);
var
  Cabfila, CabCol: PNodoCabecera;
  ValorFila, ValorColumna: string;
  valorNodoInterno: Integer;
begin

  valorFila := Trim(CampoY.Text);
  if valorFila = '' then
  begin
  ...
  valorNodoInterno := StrToInt(valorCelda.Text);

  CabFila := ObtenerOCrearCabeceraFila(valorFila);
  CabCol  := ObtenerOCrearCabeceraColumna(valorColumna);

  InsertarEnSubmatriz(CabFila, CabCol, valorNodoInterno);
...
// Funcion para buscar o crear una cabecera en la fila
function TForm1.ObtenerOCrearCabeceraFila(Fila: string): PNodoCabecera;
var
...
begin
  ...
  // Recorrido de la lista cabecera filas
  ...
  begin
    ...
    // Enlazamos el nuevo nodo en la lista
...
// Funcion para buscar o crear nodo cabecera en columna
function TForm1.ObtenerOCrearCabeceraColumna(Columna: string): PNodoCabecera;
var
  Anterior, Actual, Nueva: PNodoCabecera;
...
begin
  ...
  // Recorrido en la lista de columnas
...
// Insertar un nodo en la submatriz de una fila especifica
procedure TForm1.InsertarEnSubmatriz(CabFila, CabCol: PNodoCabecera; Detalle: Integer);
var
  Actual, Anterior, Nuevo: PNodoInterno;
  ColObjetivo: string;

begin
  // Obtenemos el primer nodo interno de la fila
  Actual := PNodoInterno(CabFila^.Acceso);
  Anterior := nil;
  ColObjetivo := CabCol^.Columna;

  // Recorremos la lista de nodos internos
  ...
  // Si ya existe un nodo en una posición, se actualizan el valor
